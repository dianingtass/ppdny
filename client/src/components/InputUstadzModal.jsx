...
export default function UstadzModal({ isOpen, onClose, isEditing, editData, onSubmit, saving, userRole, viewOnly }) {
  ...
  const inputClassName = "w-full p-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 outline-none transition-colors";
      ...
      <div onClick={(e) => e.stopPropagation()} className="cursor-default bg-white w-full max-w-lg rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="p-5 border-b border-gray-100 flex justify-between items-center bg-gray-50 rounded-t-2xl">
          <h3 className="font-bold text-gray-800 text-lg flex items-center gap-2">
            ...
            {isReadOnly ? "Detail Data Ustadz" : isEditing ? "Edit Data Ustadz" : "Tambah Ustadz Baru"}
          </h3>
        ...
        <div className="p-6 overflow-y-auto">
          <form id="ustadzForm" onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <div className="relative">
                  <input type="text" name="nip" disabled={isReadOnly} className={`pl-9 ${inputClassName}`} value={formData.nip} onChange={handleChange} placeholder="Nomor Induk Pegawai" />
                  <Briefcase className="absolute left-3 top-3 text-gray-400" size={16} />
                </div>
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Nama Lengkap {!isReadOnly && <span className="text-red-500">*</span>}</label>
                <input type="text" name="nama" required={!isReadOnly} disabled={isReadOnly} className={inputClassName} value={formData.nama} onChange={handleChange} placeholder="Nama Ustadz/Ustadzah" />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Email</label>
                <div className="relative">
                  <input type="email" name="email" disabled={isReadOnly} className={`pl-9 ${inputClassName}`} value={formData.email} onChange={handleChange} placeholder="Opsional" />
              ...
              <div>
                ...
                <div className="relative">
                  <input type="text" name="no_hp" disabled={isReadOnly} className={`pl-9 ${inputClassName}`} value={formData.no_hp} onChange={handleChange} placeholder="Opsional" />
            ...
            <div>
              ...
              <select name="jenis_kelamin" disabled={isReadOnly} className={inputClassName} value={formData.jenis_kelamin} onChange={handleChange}>
            ...
            <div>
              ...
              <div className="relative">
                <textarea name="alamat" rows="2" disabled={isReadOnly} className={`pl-9 ${inputClassName} resize-none`} value={formData.alamat} onChange={handleChange} placeholder="Alamat lengkap (opsional)..." />
            ...
            {!isReadOnly && (
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{isEditing ? "Password Baru (Opsional)" : "Password (Default: 12345678)"}</label>
                <input type="password" name="password" className={inputClassName} value={formData.password} onChange={handleChange} placeholder="******" />
              </div>
            )}
          </form>
        </div>
