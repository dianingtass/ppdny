...
export default function ListAnakModal({ isOpen, onClose, ortuData, onAssignClick, onEditClick, refreshTrigger }) {
  ...
  useEffect(() => {
    if (isOpen && ortuData) fetchAnak();
  }, [isOpen, ortuData, refreshTrigger]);

  const fetchAnak = async () => {
    setLoading(true);
    try {
      const { data } = await api.get(`/${rolePrefix}/orangtua/${ortuData.id}/anak`);
        ...
        <div onClick={(e) => e.stopPropagation()} className="cursor-default fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div className="px-6 py-5">
              <p className="text-gray-600">Yakin ingin memutus relasi anak ini?</p>
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 [scrollbar-width:none]">
          {loading ? (
            ...
            <div className="text-center py-20 text-gray-400">
              ...
              <p className="text-sm">Belum ada anak yang terhubung.</p>
            </div>
          )}
        </div>
      </div>
