...
export default function ListOrtuModal({ isOpen, onClose, santriData, onAssignClick, refreshTrigger }) {
  ...
  useEffect(() => {
    if (isOpen && santriData) fetchOrtu();
  }, [isOpen, santriData, refreshTrigger]);

  const fetchOrtu = async () => {
    setLoading(true);
    try {
      const { data } = await api.get(`/${rolePrefix}/santri/${santriData.id}/ortu`);
  ...
  return (
    <div onClick={onClose} className="fixed cursor-pointer inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      ...
      {/* Confirm Dialog */}
      {confirmRelasi.open && (
        <div onClick={(e) => e.stopPropagation()} className="cursor-default fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div className="px-6 py-5">
              <p className="text-gray-600">Yakin ingin memutus relasi wali ini?</p>
            </div>
            <div className="px-6 pb-5 flex justify-end gap-3">
              <button
                onClick={() => setConfirmRelasi({ open: false, id: null })}
                className="px-5 py-2.5 bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 rounded-xl font-semibold text-sm transition"
              >
                Batal
              </button>
              <button
      ...
      <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl flex flex-col max-h-[85vh]">
        <div className="p-5 border-b border-gray-100 flex justify-between items-center bg-gray-50 rounded-t-2xl">
          <div>
            <h3 className="font-bold text-gray-800 text-lg">Daftar Wali / Orang Tua</h3>
            <p className="text-xs text-gray-500">Santri: {santriData?.nama}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-4 border-b border-gray-100 flex justify-end bg-white">
          <button
            onClick={() => onAssignClick(santriData)}
        ...
        <div className="flex-1 overflow-y-auto p-4 [scrollbar-width:none]">
          {loading ? (
            ...
            <div className="text-center py-20 text-gray-400">
              ...
              <p className="text-sm">Belum ada wali yang terhubung.</p>
            </div>
          )}
        </div>
      </div>
