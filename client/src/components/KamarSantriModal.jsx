...
export default function KamarSantriModal({ isOpen, onClose, kamarData, onAssignClick, refreshTrigger, rolePrefix = "pengurus" }) {
  ...
  const fetchSantri = async () => {
    ...
    try {
      ...
      const { data } = await api.get(`/${rolePrefix}/kamar/${kamarData.id}/santri`);
  ...
  const confirmDelete = async () => {
    setIsRemoving(true);
    try {
      const { data } = await api.delete(`/${rolePrefix}/kamar/${kamarData.id}/santri/${deleteConfirm.id}`);
      if (data.success) {
        showAlert("success", "Santri berhasil dikeluarkan dari kamar");
  ...
  return (
    <div onClick={onClose} className="fixed cursor-pointer inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div onClick={(e) => e.stopPropagation()} className="cursor-default bg-white w-full max-w-2xl rounded-2xl shadow-2xl flex flex-col max-h-[85vh] relative">
