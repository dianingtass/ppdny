...
import { User, X, Search, Loader2, CheckCircle, Plus, Users } from "lucide-react";
...
export default function AssignRelasiModal({ isOpen, onClose, mode, baseData, onSubmit, saving }) {
    ...
    const delayDebounce = setTimeout(async () => {
      if (formStep === 1 && searchQuery.length >= 3) {
        ...
        try {
          ...
          const res = await api.get(`/${rolePrefix}/orangtua/search?role=${targetRole}&q=${searchQuery}`);
  ...
  }, [searchQuery, formStep, mode, rolePrefix]);

  const handleSelectUser = (user) => {
    setFormData({
      id_selected: user.id,
      nama: user.nama,
      no_hp: user.no_hp ?? user.nip,
      hubungan: "",
    });
    setIsManualInput(false);
    setFormStep(2);
  ...
  return (
    <div onClick={onClose} className="fixed cursor-pointer inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div onClick={(e) => e.stopPropagation()} className="cursor-default bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
          ...
          {formStep === 1 && (
            <div className="space-y-4">
              <div className="relative">
                ...
                {isSearching && (
                  <Loader2 className="absolute right-3 top-3.5 text-green-500 animate-spin" size={20} />
                )}
              </div>
                    ...
                    >
                      <div>
                        ...
                        <p className="text-xs text-gray-500">
                          {isCariOrtu ? `No HP: ${user.no_hp}` : `NIS: ${user.nip}`}
                        </p>
                      </div>
              ...
              {/* Input Manual hanya untuk buat akun Wali baru */}
              {isCariOrtu && (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full p-3 border border-gray-200 rounded-xl outline-none ${
                    !isManualInput ? "bg-gray-100 text-gray-500" : "focus:ring-2 focus:ring-green-500"
                  }`}
                />
              </div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  type="submit"
                  disabled={saving}
                  className="flex-1 px-4 py-3 bg-green-600 text-white font-medium rounded-xl hover:bg-green-700 flex justify-center items-center"
                >
