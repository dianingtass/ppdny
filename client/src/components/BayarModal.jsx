  ...
  return (
    <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col">
        <div className="p-6 pb-0"><h3 className="text-xl font-bold text-gray-900">Tambah Data Pembayaran</h3></div>
          ...
          <div className="border-2 border-dashed border-gray-300 rounded-xl p-8 text-center hover:bg-gray-50 transition cursor-pointer flex flex-col items-center justify-center gap-3 group" onClick={() => fileInputRef.current.click()}>
            ...
            {file ? (
              ...
              <>
                ...
                <div><h4 className="text-sm font-bold text-gray-800">Unggah Bukti Pembayaran</h4><p className="text-xs text-gray-500 mt-1">Klik atau seret file untuk mengunggah bukti.</p></div>
                <div className="mt-2 bg-gray-100 text-gray-700 px-4 py-2 rounded-lg text-xs font-semibold">Pilih File</div>
              </>
            )}
          </div>
        </div>
        <div className="p-6 pt-0 flex gap-3">
          <button onClick={onClose} disabled={saving} className="flex-1 py-3 bg-gray-100 text-gray-700 font-semibold rounded-xl">Tutup</button>
          <button onClick={handleSubmit} disabled={saving || !file} className="flex-1 py-3 bg-green-600 text-white font-semibold rounded-xl flex items-center justify-center">
            {saving && <Loader2 className="animate-spin mr-2" size={20} />} {saving ? "Mengirim..." : "Kirim Pembayaran"}
          </button>
        </div>
      </div>
    </div>
