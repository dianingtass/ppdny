...
export default function InputTagihanModal({ isOpen, onClose, isEditing, editData, onSubmit, rolePrefix = "pengurus" }) {
  ...
  useEffect(() => {
    ...
    if (isOpen) {
        ...
        if (isEditing && editData) {
            ...
            setFormData({ 
                nama_tagihan: "", id_jenis_tagihan: "", nominal: "", 
  ...
  const toggleSantri = (id) => {
    setSelectedSantri(prev => prev.includes(id) ? prev.filter(s => s !== id) : [...prev, id]);
  };

  const handleSelectAll = (e) => {
    setIsSelectAll(e.target.checked);
    if (e.target.checked) setSelectedSantri([]);
  };
  ...
  // Clean code: Safe navigation `?.toLowerCase()`
  const filteredSantri = allSantri.filter(s => 
      s?.nama?.toLowerCase().includes(searchTerm.toLowerCase()) && !selectedSantri.includes(s.id)
  );
  ...
  return (
    <div onClick={onClose} className="fixed cursor-pointer inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div onClick={(e) => e.stopPropagation()} className="cursor-default bg-white w-full max-w-xl rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto space-y-4">
            <div className="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select className="w-full p-2.5 border border-gray-200 rounded-xl bg-white focus:ring-2 focus:ring-green-500 outline-none" value={formData.id_jenis_tagihan} onChange={e => setFormData({...formData, id_jenis_tagihan: e.target.value})}>
            ...
            <div className="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" className="w-full p-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 outline-none" value={formData.tanggal_tagihan} onChange={e => setFormData({...formData, tanggal_tagihan: e.target.value})}/>
                </div>
                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Jatuh Tempo</label>
                    <input type="date" className="w-full p-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 outline-none" value={formData.batas_pembayaran} onChange={e => setFormData({...formData, batas_pembayaran: e.target.value})}/>
                </div>
            </div>
                                ...
                                {searchTerm && (
                                    <div className="absolute top-full left-0 right-0 bg-white border border-gray-100 shadow-lg rounded-xl mt-1 max-h-40 overflow-y-auto z-10">
                                        {filteredSantri.map(s => (
                                            <button key={s.id} type="button" onClick={() => { toggleSantri(s.id); setSearchTerm(""); }} className="w-full text-left px-4 py-2 hover:bg-green-50 text-sm text-gray-700">
        ...
        <div className="p-5 border-t border-gray-100 bg-gray-50 rounded-b-2xl flex justify-end gap-3">
            <button onClick={onClose} type="button" className="px-5 py-2.5 bg-white border border-gray-300 text-gray-700 font-medium rounded-xl hover:bg-gray-100">Batal</button>
            <button onClick={handleSubmit} className="px-5 py-2.5 bg-green-600 text-white font-medium rounded-xl hover:bg-green-700 flex items-center">
