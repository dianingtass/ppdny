...
export default function AssignKamarModal({
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!formData.id_santri || !formData.id_kamar)
      return showAlert("error", "Pilih Santri!");
    onSubmit(formData);
  };

  const displayKamarName =
    editData?.kamar?.kamar || preSelectedKamar?.kamar || "Loading...";
  const displayLokasi =
      ...
      <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
        <div className="p-5 border-b border-gray-100 flex justify-between items-center bg-gray-50 rounded-t-2xl">
          <h3 className="font-bold text-gray-800 text-lg flex items-center gap-2">
            ...
            {isEditing ? "Pindah Kamar Santri" : "Tambah Santri ke Kamar"}
          </h3>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    value={`${displayKamarName} (${displayLokasi})`}
                  />
                  <Home
          ...
          <div className="pt-4 flex gap-3">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 py-2.5 bg-gray-100 text-gray-700 rounded-xl hover:bg-gray-200 font-medium transition"
            >
              Batal
            </button>
            <button
              type="submit"
              disabled={saving}
              className="flex-1 py-2.5 bg-green-600 text-white rounded-xl hover:bg-green-700 flex justify-center items-center font-medium shadow-lg transition"
            >
              {saving ? (
