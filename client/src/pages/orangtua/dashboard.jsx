    ...
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      <div className="max-w-6xl mx-auto px-4 -mt-12">
        <div className="bg-white rounded-2xl shadow-xl p-6 flex flex-col md:flex-row items-center gap-6 border border-white">
          ...
          <button onClick={() => navigate("/orangtua/keuangan")} className="w-full md:w-auto bg-green-600 text-white px-6 py-3 rounded-2xl font-bold shadow-lg shadow-green-200 active:scale-95 transition">
            Bayar Tagihan
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-8">
              ...
              <div className="space-y-3">
                {data.pengaduan_terakhir.length > 0 ? data.pengaduan_terakhir.map(p => (
                  <div key={p.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl">
                    ...
                    <span className={`px-2 py-1 rounded-lg text-[10px] font-black uppercase ${p.status === 'Selesai' ? 'bg-green-100 text-green-700' : 'bg-green-100 text-green-700'}`}>
                      {p.status}
                    </span>
                  </div>
          ...
          <div className="space-y-6 mb-10">
            <div className="bg-white rounded-2xl shadow-lg shadow-green-100 border border-green-50 p-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
              ...
              <div className="grid grid-cols-1 gap-4">
                ...
                <div className="bg-orange-50 p-4 rounded-2xl border border-orange-100">
                  ...
                  <p className="text-sm font-bold text-gray-800">{data.kesehatan.screening?.diagnosa?.replaceAll('_', ' ') || 'Belum Screening'}</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="fixed bottom-4 left-4 right-4 bg-white rounded-2xl shadow-2xl p-4 z-50 border border-gray-100 md:hidden">
        <div className="flex justify-around">
          <button onClick={() => navigate('/orangtua')} className='flex flex-col items-center p-2 text-green-600'>
            <Home size={24} />
