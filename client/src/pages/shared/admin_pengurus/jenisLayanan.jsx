...
export default function JenisLayananPage({ rolePrefix }) {
  ...
  const fetchLayanan = async () => {
    setLoading(true);
    try {
      const res = await api.get(`/${rolePrefix}/jenis-layanan`, { params: { search } });
  ...
  useEffect(() => {
    const t = setTimeout(() => { fetchLayanan(); jump(1); }, 500);
  ...
  const handleSubmit = async (formData) => {
    ...
    try {
      if (isEditing) {
        await api.put(`/${rolePrefix}/jenis-layanan/${selectedData.id}`, formData);
        showAlert("success", "Jenis layanan berhasil diperbarui");
      } else {
        await api.post(`/${rolePrefix}/jenis-layanan`, formData);
  ...
  const handleDelete = (item) => setDeleteModal({ isOpen: true, id: item.id, name: `Layanan "${item.nama_layanan}"` });

  const confirmDelete = async () => {
    setIsDeleting(true);
    try {
      await api.delete(`/${rolePrefix}/jenis-layanan/${deleteModal.id}`);
  ...
  return (
    <div className="space-y-6 relative">
      ...
      <div className="flex justify-between items-center">
        <div><h1 className="text-2xl font-bold text-gray-800">Jenis Layanan</h1><p className="text-gray-500 text-sm">Master data layanan dan biaya</p></div>
