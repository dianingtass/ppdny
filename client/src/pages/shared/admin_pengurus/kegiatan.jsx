...
export default function KegiatanPage({ rolePrefix }) {
  ...
  const [selectedSkalaType, setSelectedSkalaType] = useState(""); // "", "Global", "Kelas", "Kamar"
  const [selectedSpecificTarget, setSelectedSpecificTarget] = useState(""); // e.g. "7A", "Al-Ghozali"

  // Ambil list unik kelas & kamar dari dataList
  ...
  const filteredData = dataList.filter((item) => {
    // Filter berdasarkan Tipe Skala (Global / Kelas / Kamar)
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await api.get(`/${rolePrefix}/kegiatan`, { params: { search, type: filterType || undefined } });
