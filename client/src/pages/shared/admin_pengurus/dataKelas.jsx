...
export default function DataKelasPage({ rolePrefix }) {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await api.get(`/${rolePrefix}/kelas`, { params: { search } });
  ...
  const handleSubmitKelas = async (formData) => {
    ...
    try {
      if (modalKelas.isEditing) {
        await api.put(`/${rolePrefix}/kelas/${modalKelas.data.id}`, formData);
        showAlert("success", "Kelas diperbarui");
      } else {
        await api.post(`/${rolePrefix}/kelas`, formData);
  ...
  const confirmDelete = async () => {
    setIsDeleting(true);
    try {
      await api.delete(`/${rolePrefix}/kelas/${deleteModal.id}`);
                    ...
                    <tr key={item.id} className="hover:bg-gray-50 transition">
                      ...
                      <td className="p-4 text-center">
                        <div className="flex justify-center gap-2">
                          ...
                          <button onClick={() => setKelasSantriModal({ isOpen: true, data: item })} className="p-2 text-green-600 hover:bg-green-50 rounded-lg"><Users size={18} /></button>
            ...
            {currentData.length > 0 ? currentData.map((item) => (
              <div key={item.id} className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col gap-3">
                ...
                <div className="grid grid-cols-2 gap-3 mt-1">
                  ...
                  <button onClick={() => setKelasSantriModal({ isOpen: true, data: item })} className="py-2 bg-green-50 text-green-600 rounded-xl font-semibold text-sm flex justify-center items-center gap-2"><Users size={16} /> Santri</button>
                </div>
              </div>
