...
export default function DataStaf() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await api.get("/pimpinan/staf");
      if (res.data.success) {
        setDataList(res.data.data);
      }
    } catch (err) {
      console.error("Fetch error:", err.response?.data?.message || err.message);
      showAlert("error", "Gagal memuat data staf");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6 relative">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-800 flex items-center gap-2">Data Staf</h1>
          <p className="text-gray-500 text-sm">Lihat data administrator, pimpinan, tim kesehatan, dan pengurus</p>
        </div>
      </div>
                      ...
                      <tr key={item.id} className="hover:bg-green-50/50 transition">
                        ...
                        <td className="p-4 text-center">
                          <div className="flex items-center justify-center gap-2">
                            <button onClick={() => handleView(item)} className="p-2 text-green-600 hover:bg-green-100 rounded-lg transition" title="Lihat Detail"><Eye size={18} /></button>
                          </div>
                        </td>
                      </tr>
                    ))
            ...
            {currentData.length > 0 ? (
              currentData.map((item) => (
                <div key={item.id} className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col gap-3">
                  ...
                  <button onClick={() => handleView(item)} className="w-full py-2 bg-green-50 text-green-600 rounded-xl font-semibold text-sm flex items-center justify-center gap-2 active:scale-95 transition"><Eye size={16} /> Lihat Detail</button>
                </div>
              ))
            ) : (
