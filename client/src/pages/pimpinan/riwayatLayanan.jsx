...
export default function RiwayatLayananPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await api.get(`/pimpinan/riwayat-layanan`, { params: { search } });
    ...
    <div className="space-y-6 relative">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500 text-sm font-medium">Laporan pengajuan layanan santri</p>
        </div>
      </div>
          ...
          <div className="block md:hidden space-y-4">
            {currentData.length > 0 ? currentData.map((item) => (
              <div key={item.id} className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col gap-3">
                ...
                <button onClick={() => handleOpenDetail(item)} className="w-full py-2.5 bg-green-50 hover:bg-green-100 text-green-600 rounded-xl font-bold text-xs flex items-center justify-center gap-2 active:scale-95 transition">
      ...
      <DetailRiwayatModal isOpen={isDetailOpen} onClose={() => setIsDetailOpen(false)} data={detailData} />
    </div>
  );
}
